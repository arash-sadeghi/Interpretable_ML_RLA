...
def saveData(caller=None):
    ...
    if localMinima:
        data2BsavedStr = ["NASwG", "NASG", "NASwL", "NASL",
                          "log", "Qtable", "rewards", "eps", "alpha"]
        data2Bsaved = [NASwG, NASG, NASwL, NASL,
                       log, QtableMem, reward, eps, alpha]
    else:
        data2BsavedStr = ["NASw", "NAS", "log",
                          "Qtable", "rewards", "eps", "alpha"]
        data2Bsaved = [NASw, NAS, log, QtableMem, reward, eps, alpha]
    fileName = codeBeginTime+dirChangeCharacter+dataType+dirChangeCharacter + \
        str(it)+' '+str(sup.getTime())+' s ' + \
        ctime(TIME()).replace(':', '_')+' '
...
def keyboardInterruptHandler(signal, frame):
    saveData('interupt')
    print(colored('\t\t[+] half data saved. Time: '+str(sup.getTime()) +
          ', ratio '+str(int(sup.getTime()/FinalTime*100))+'%', 'green'))
    ans = input(colored('\t\t[+] continue/quit? [c/q]', 'green'))
    if ans == 'q':
        exit(0)
    else:
        print(colored('\t\t[+] continuing', 'green'))
...
# ...............................................................................................................................
def LOG():
    global sup, samplingPeriodSmall, it, sampled, save_csv, codeBeginTime,\
        dirChangeCharacter, FinalTime, save_tables_videos, videoList, record,\
        showFrames, SAR, localMinima, t,started_second,ROBN
    ...
    if sup.getTime() % samplingPeriodSmall == 0 and sup.getTime()-t > 1:
        ...
        if method == 'RL' or method == 'DDPG':
            if it == 0:  # save these in the first iteration only
                if sampled % 100 == 0:
                    ...
                    if save_tables_videos:
                        imsMat = [sup.swarm[0].epsilon*255,
                                  sup.swarm[0].QtableCheck*255, QtableRob0]
                    ...
                    with open(codeBeginTime+dirChangeCharacter+'SAR_robot0.npy', 'wb') as SAR_f:
                        np.save(SAR_f, SAR)
                    np.savetxt(codeBeginTime+dirChangeCharacter +
                               'SAR_robot0.csv', np.round(SAR), delimiter=",")
            ...
            if localMinima:
                ...
                NASwG[it, sampled], NASwL[it,
                                          sampled] = sup.getNAS(weighted=True)
            else:
        ...
        sampled += 1
        if sampled%100==0:
            done_precentage=sup.getTime()/FinalTime*100
            print(colored('\t[+] sim time >{:.2f}< real time passed >{:.2f}< percentage completed >{:.2f}%<'\
                .format(sup.getTime(),TIME()-started_second,done_precentage),'yellow'),end='\r')
            if done_precentage%10==0 and method == 'DDPG':
                sup.memory.save()
                for i in range(ROBN):
                    sup.swarm[i].Agent.save_models()
        t = sup.getTime()
# ...............................................................................................................................
def init_params():
    global comment
    ...
    for flag_name, flag_value in flags:
        globals()[flag_name] = True if (flag_value ==
                                        "True" or flag_value == "true") else False

    if method == "RL":
        comment = paramReductionMethod+' ' + \
            str(PRMparameter)+' '+comment+' noise '+str(noise)
    else:
...
if __name__ == "__main__" or True:
    ...
    print(colored('[+] paramReductionMethod', 'green'),
          paramReductionMethod, PRMparameter)
    ...
    if method== 'DDPG':
        ANNdir=codeBeginTime+dirChangeCharacter+'ANN'
        os.makedirs(ANNdir)
